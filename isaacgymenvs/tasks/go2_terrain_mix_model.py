...
class Go2Terrain(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.num_act_obs = self.num_dyn_obs - 9 - 3
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.chain = pk.build_chain_from_urdf(open(asset_path, mode="rb").read())
    ...
    def _diff_reward_raibert_heuristic(self, full_obs):
        ...
        #cur_footsteps_translated = full_obs[:, -12:].reshape(-1, 4, 3)
        #foot_positions = full_obs[:, -12:].reshape(-1, 4, 3)
        #cur_footsteps_translated = foot_positions - self.prev_base_pos.unsqueeze(1)
        #footsteps_in_body_frame = torch.zeros(self.num_envs, 4, 3, device=self.device)
        #for i in range(4):
        #    footsteps_in_body_frame[:, i, :] = quat_apply_yaw(quat_conjugate(self.base_quat),
        #                                                      cur_footsteps_translated[:, i, :])
        #footsteps_in_body_frame = full_obs[:, -16:-4].reshape(-1, 4, 3)

        ##base_quat = full_obs[:, -8:-4]
