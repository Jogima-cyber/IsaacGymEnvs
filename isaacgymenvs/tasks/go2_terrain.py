...
class Go2Terrain(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.num_act_obs = self.num_dyn_obs - 16 - 3
        ...
        self.episode_sums = {"lin_vel_xy": torch_zeros(), "ang_vel_z": torch_zeros(), "torques": torch_zeros(),
                                ...
                                "lin_vel_z": torch_zeros(), "dof_vel": torch_zeros(), "ang_vel_xy": torch_zeros(),
    ...
    def observe_misc(self):
        ...
        return torch.cat((self.projected_gravity,  # 3 # projected gravity, an image of the orientation (t)
                          ...
                          self.dof_vel * self.dof_vel_scale, # 12 # ((self.dof_pos - self.last_dof_pos[:, :, 0]) / self.dt) * self.dof_vel_scale, # joint velocities (t)
    ...
    def observe_foot_positions(self):
        cur_footsteps_translated = self.foot_positions - self.base_pos.unsqueeze(1)
        footsteps_in_body_frame = torch.zeros(self.num_envs, 4, 2, device=self.device)
        for i in range(4):
            #footsteps_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, cur_footsteps_translated[:, i, :])
            footsteps_in_body_frame[:, i, :] = quat_apply_yaw(quat_conjugate(self.base_quat),
    ...
    def compute_reward_CaT(self):
        ...
        ############################################
        """rewards = [rew_lin_vel_xy, rew_ang_vel_z, rew_torque, rew_action_rate, rew_airTime, rew_foot2contact, rew_raibert_heuristic, rew_stand_still, rew_orient, rew_lin_vel_z, rew_dof_vel]
        rew_buf_pos = torch.zeros_like(self.rew_buf)
        rew_buf_neg = torch.zeros_like(self.rew_buf)
        for rew in rewards:
            if torch.sum(rew) >= 0:
                rew_buf_pos += rew
            elif torch.sum(rew) <= 0:
                rew_buf_neg += rew
        self.rew_buf[:] = rew_buf_pos[:] * torch.exp(rew_buf_neg[:] / 0.02)"""
        ############################################
        if self.useConstraints == "cat":
    ...
    def _diff_reward_raibert_heuristic(self, full_obs):
        ...
        footsteps_in_body_frame = full_obs[:, -16:-8].reshape(-1, 4, 2)
    ...
    def diffRecalculateReward(self, full_obs, actions, last_actions, last_last_actions, imagined_trajs = None):
        # Velocity tracking reward
        actions = actions * 2.
        last_actions = last_actions * 2.
        last_last_actions = last_last_actions * 2.
        ...
        rew_raibert_heuristic = self._diff_reward_raibert_heuristic(full_obs)
        rew_raibert_heuristic = rew_raibert_heuristic * self.rew_scales["raibertHeuristic"]
        ...
        rew_dof_vel = torch.sum(torch.square(full_obs[:, 24:36] / self.dof_vel_scale), dim=1) * self.rew_scales["dof_vel"]
        rew_torque = torch.sum(torch.square(full_obs[:, -28:-16] / self.torques_scale), dim=1) * self.rew_scales["torque"]
        rew_ang_vel_xy = torch.sum(torch.square(full_obs[:, 3:5] / self.ang_vel_scale), dim=1) * self.rew_scales["ang_vel_xy"]
        rew_airTime = torch.sum((full_obs[:, 48:52] - 0.25) * self.prev_contacts_touchdown, dim=1) * self.rew_scales["air_time"] # /!\

        self.diff_reward_rew_action_rate = rew_action_rate.clone()

        phases = 1 - torch.abs(1.0 - torch.clip((self.prev_foot_indices * 2.0) - 1.0, 0.0, 1.0) * 2.0)
        foot_height = (full_obs[:, -4:]).view(self.num_envs, -1) # - reference_heights
        target_height = 0.08 * phases + 0.02 # offset for foot radius 2cm
        rew_foot_clearance = torch.square(target_height - foot_height) * (1 - self.desired_contact_states)
        rew_feet_clearance = torch.sum(rew_foot_clearance, dim=1) * self.rew_scales["feetClearance"]
        ...
        foot_forces = full_obs[:, -8:-4] / self.foot_forces_scale
        desired_contact = self.desired_contact_states
        rew_tracking_contacts_shaped_force = 0.
        for i in range(4):
            rew_tracking_contacts_shaped_force += - (1 - desired_contact[:, i]) * (
        ...
        diff_rew = rew_lin_vel_xy + rew_ang_vel_z + rew_raibert_heuristic + rew_action_rate + rew_orient + rew_lin_vel_z + rew_dof_vel + rew_torque + rew_ang_vel_xy + rew_airTime + rew_feet_clearance + rew_foot2contact + rew_tracking_contacts_shaped_force
        diff_rew = torch.clip(diff_rew, min=0., max=None)

        return diff_rew

    def full2partial_state(self, full_obs):
        return full_obs[:, 3:-16]

####################
# jit functions
####################
"""@torch.jit.script
def diffRecalculateReward(
  full_obs,
  actions,
  last_actions,
  last_last_actions,
  ...
  rew_scales, #
    ...
    lin_vel_error = torch.sum(torch.square(prev_commands[:, :2] - full_obs[:, :2] / lin_vel_scale), dim=1)
    ang_vel_error = torch.square(self.prev_commands[:, 2] - full_obs[:, 5] / ang_vel_scale)
    rew_lin_vel_xy = torch.exp(-lin_vel_error / self.lin_vel_delta) * rew_scales["lin_vel_xy"]
    rew_ang_vel_z = torch.exp(-ang_vel_error / self.ang_vel_delta) * rew_scales["ang_vel_z"]

    rew_raibert_heuristic, footsteps_z_in_world_frame = self._diff_reward_raibert_heuristic(full_obs)
    rew_raibert_heuristic = rew_raibert_heuristic * rew_scales["raibertHeuristic"]
    ...
    phases = 1 - torch.abs(1.0 - torch.clip((self.prev_foot_indices * 2.0) - 1.0, 0.0, 1.0) * 2.0)
    foot_height = footsteps_z_in_world_frame # (full_obs[:, -4:]).view(self.num_envs, -1) # - reference_heights
    target_height = 0.08 * phases + 0.02 # offset for foot radius 2cm
    rew_foot_clearance = torch.square(target_height - foot_height) * (1 - self.desired_contact_states)
    rew_feet_clearance = torch.sum(rew_foot_clearance, dim=1) * self.rew_scales["feetClearance"]
    ...
    foot_forces = full_obs[:, -9:-5] / self.foot_forces_scale
    desired_contact = self.desired_contact_states
    rew_tracking_contacts_shaped_force = 0.
    for i in range(4):
        rew_tracking_contacts_shaped_force += - (1 - desired_contact[:, i]) * (
...
@torch.jit.script
def _diff_reward_raibert_heuristic(
  full_obs,
  prev_commands, #
):
    commands = prev_commands

    base_quat = full_obs[:, -5:-1]
    base_z = full_obs[:, -1]
    feet_names = ["FL_foot", "FR_foot", "RL_foot", "RR_foot"]
    tg_batch = self.chain.forward_kinematics(full_obs[:, 12:24])
    footsteps_in_body_frame_prime = torch.stack([tg_batch[foot_name].get_matrix()[:, :3, 3] for foot_name in feet_names], dim = 1)
    footsteps_in_body_frame = torch.zeros(self.num_envs, 4, 3, device=self.device)
    footsteps_z_in_world_frame = torch.zeros(self.num_envs, 4, device=self.device)
    for i in range(4):
        #footsteps_in_body_frame[:, i, :] = quat_apply_yaw(quat_conjugate(self.prev_base_quat.clone()),
        #                                                  quat_rotate(self.prev_base_quat.clone(), footsteps_in_body_frame_prime[:, i, :]))

        footsteps_in_world_orient = quat_rotate(self.prev_base_quat.clone(), footsteps_in_body_frame_prime[:, i, :])
        footsteps_in_body_frame[:, i, :] = quat_apply_yaw(quat_conjugate(base_quat), footsteps_in_world_orient)
        footsteps_z_in_world_frame[:, i] = footsteps_in_world_orient[:, -1] + base_z
    # nominal positions: [FR, FL, RR, RL]
    desired_stance_width = 0.25
    desired_ys_nom = torch.tensor([desired_stance_width / 2, -desired_stance_width / 2, desired_stance_width / 2, -desired_stance_width / 2], device=self.device).unsqueeze(0)
    ...
    return reward, footsteps_z_in_world_frame"""
